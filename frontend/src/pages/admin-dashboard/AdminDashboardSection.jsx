...
export default function AdminDashboardSection({
  ...
  const displayUserName = (u) => {
    ...
    if (n) return n;
    return u.contactEmail || u.contactPhone || u.userId || t('admin.dashboard.defaultUser');
  };
  ...
  return (
    <>
      <section className="mb-4 rounded-xl border border-gray-200/90 bg-white px-4 py-3 shadow-sm sm:px-5 sm:py-3.5">
        <h1 className="text-lg font-semibold tracking-tight text-[#1e4732] sm:text-xl">{t('admin.dashboard.welcome')}</h1>
        <p className="mt-1 max-w-2xl text-xs leading-relaxed text-gray-600 sm:text-sm">{t('admin.dashboard.subtitle')}</p>
      </section>
      ...
      <section className="mb-5 grid grid-cols-1 gap-2.5 sm:grid-cols-2">
        <StatCard
          ...
          subtitle={t('admin.dashboard.totalListingsSub')}
          accent="emerald"
        ...
        <StatCard
          ...
          subtitle={t('admin.dashboard.activeListingsSub')}
          accent="teal"
          iconPath="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"
          footer={
            inactiveListings > 0 ? (
              <p className="text-[11px] text-gray-500 sm:text-xs">
                {t('admin.dashboard.inactiveFooter', { count: inactiveListings.toLocaleString() })}
              </p>
            ) : null
          }
        />
      ...
      <section className="mb-5 grid grid-cols-2 gap-2.5 lg:grid-cols-3">
        ...
        />
        <StatCard
          title={t('admin.dashboard.totalEarnings')}
          value={formatBdt(stats?.totalEarningsBdt ?? 0, t)}
          accent="sky"
      ...
      <div className="grid grid-cols-1 gap-4 sm:gap-5 xl:grid-cols-12 xl:gap-5 xl:[&>section]:min-h-0">
        <section className="flex min-h-0 flex-col rounded-xl border border-gray-200/90 bg-white p-3.5 shadow-sm sm:p-5 xl:col-span-7 xl:h-full xl:min-h-[22rem]">
          <div className="mb-3 flex min-h-10 shrink-0 items-center justify-between gap-3 sm:min-h-11">
            <h2 className="min-w-0 flex-1 truncate text-base font-semibold leading-snug text-gray-900">{t('admin.dashboard.recentListings')}</h2>
            <button
              type="button"
              onClick={() => {
            ...
            <div className="min-h-0 flex-1 space-y-3 overflow-y-auto pr-0.5 [-ms-overflow-style:none] [scrollbar-width:thin] [&::-webkit-scrollbar]:w-1.5">
              {recentListingsFeed.length === 0 ? (
                <p className="flex min-h-[12rem] flex-1 items-center justify-center rounded-lg bg-gray-50 px-2 py-7 text-center text-sm text-gray-500">
                  {t('admin.dashboard.noListings')}
                </p>
              ) : (
                recentListingsFeed.map((row) => {
                  const badge = listingFeedBadge(row.listingStatus, t);
                  return (
                    <div
                      key={row.listingId}
                    ...
                    >
                      <div className="aspect-[16/10] w-full shrink-0 overflow-hidden rounded-lg bg-gray-200 sm:aspect-auto sm:h-28 sm:w-36">
                        <img
                          ...
                          alt={row.title || t('admin.dashboard.listingAlt')}
                      ...
                      <div className="flex min-w-0 flex-1 flex-col justify-between">
                        <div>
                          ...
                          <p className="mt-0.5 text-xs font-medium text-emerald-800 sm:text-sm">
                            {formatBdt(row.rent, t)}
                            {t('admin.dashboard.perMonth')}
                          </p>
                          <p className="mt-0.5 text-xs text-gray-500">{toLabel(row.areaName, t)}</p>
                        </div>
                        <div className="mt-3 flex flex-wrap items-center gap-2">
                          ...
                          >
                            {selectingListingId === row.listingId ? t('admin.dashboard.loading') : t('admin.dashboard.viewDetails')}
                          </button>
                        </div>
                      </div>
                    </div>
        ...
        <section className="flex min-h-0 flex-col rounded-xl border border-gray-200/90 bg-white p-3.5 shadow-sm sm:p-5 xl:col-span-5 xl:h-full xl:min-h-[22rem]">
          <div className="mb-3 flex min-h-10 shrink-0 items-center justify-between gap-3 sm:min-h-11">
            <h2 className="min-w-0 flex-1 truncate text-base font-semibold leading-snug text-gray-900">{t('admin.dashboard.newUsers')}</h2>
            <button
              type="button"
              onClick={() => {
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-xs text-gray-500">
                          {u.role === 'OWNER'
                            ? t('admin.dashboard.propertyOwner')
                            : u.role === 'TENANT'
        ...
        <section className="flex min-h-0 flex-col rounded-xl border border-gray-200/90 bg-white p-3.5 shadow-sm sm:p-5 xl:col-span-7 xl:h-full xl:min-h-[22rem]">
          <div className="mb-3 flex min-h-10 shrink-0 items-center justify-between gap-3 sm:min-h-11">
            <h2 className="min-w-0 flex-1 truncate text-base font-semibold leading-snug text-gray-900">{t('admin.dashboard.recentPayments')}</h2>
            <button
              type="button"
              onClick={() => {
                ...
                {recentPaymentsFeed.map((p) => (
                  <li key={p.id} className="flex flex-wrap items-center justify-between gap-2 py-2.5 text-sm">
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="truncate text-xs text-gray-500">{p.description || toLabel(p.type, t)}</p>
                    </div>
                    <div className="shrink-0 text-right">
                      <p className="font-semibold text-gray-900">{formatBdt(p.amount, t)}</p>
                      <p className="text-xs text-gray-400">{formatRelativeTime(p.createdAt, t)}</p>
                    </div>
                  </li>
                ))}
              </ul>
        ...
        <section className="relative flex min-h-0 flex-col overflow-hidden rounded-2xl border border-emerald-100/80 bg-gradient-to-br from-white via-emerald-50/[0.35] to-teal-50/[0.45] p-3.5 shadow-sm sm:p-5 xl:col-span-5 xl:h-full xl:min-h-[22rem]">
          ...
          <div className="mb-3 flex min-h-10 shrink-0 items-center justify-between gap-3 sm:min-h-11">
            <h2 className="min-w-0 flex-1 truncate text-base font-bold leading-snug text-gray-900">{t('admin.dashboard.systemAnalytics')}</h2>
          ...
          <div className="flex min-h-0 flex-1 flex-col">
            <div className="flex min-h-0 flex-1 flex-col space-y-3 overflow-y-auto pr-0.5 [-ms-overflow-style:none] [scrollbar-width:thin] [&::-webkit-scrollbar]:w-1.5">
              <div className="inline-flex shrink-0 rounded-xl border border-emerald-200/80 bg-white p-0.5 shadow-sm">
                ...
                >
                  {t('admin.dashboard.propertiesTab')}
                </button>
              </div>
              ...
              <div className="flex min-h-0 flex-1 flex-col gap-3 sm:flex-row sm:items-stretch sm:justify-between">
                <div className="flex min-h-0 min-w-0 flex-1 flex-col rounded-xl border border-emerald-100/80 bg-white/90 px-3 py-3 shadow-sm">
                  <p className="text-[11px] font-semibold uppercase tracking-wide text-slate-500">
                    {analyticsTab === 'users' ? t('admin.dashboard.newRegistrations') : t('admin.dashboard.listingActivity')}
                  </p>
                  ...
                  <p className="mt-1 text-xs text-slate-500 sm:text-sm">
                    {analyticsTab === 'users'
                      ? t('admin.dashboard.activeUsersToday', { count: (stats?.activeToday ?? 0).toLocaleString() })
                      : t('admin.dashboard.activeTotal', {
                          active: activeListings.toLocaleString(),
                ...
                <div className="flex shrink-0 flex-col justify-center rounded-xl border border-emerald-200/80 bg-gradient-to-br from-emerald-50 to-teal-50 px-3 py-2.5 text-right shadow-sm sm:max-w-[11rem] sm:self-stretch">
                  <p className="text-[11px] font-semibold text-emerald-800">
                    {analyticsTab === 'users'
                      ? t('admin.dashboard.weekChange', { sign: userWeekPct >= 0 ? '+' : '', pct: userWeekPct })
                      : t('admin.dashboard.weekChange', { sign: listingWeekPct >= 0 ? '+' : '', pct: listingWeekPct })}
                  </p>
                  ...
                  <p className="text-[11px] font-medium text-emerald-800">
                    {analyticsTab === 'users' ? t('admin.dashboard.registeredUsers') : t('admin.dashboard.totalListingsAllTime')}
                  </p>
                </div>
              </div>
            </div>
