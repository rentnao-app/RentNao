...
const RENTAL_TYPE_OPTIONS = [
  { value: 'residential', label: 'Residential', description: 'Homes, flats, and apartments for tenants.' },
  { value: 'commercial', label: 'Commercial', description: 'Office, retail, warehouse, and service spaces.' },
];
...
function RentalTypeSelector({ value, onChange }) {
  return (
    <div>
      <p className="block text-sm font-medium text-gray-700 mb-2">Space for rent</p>
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        {RENTAL_TYPE_OPTIONS.map((option) => {
          ...
          return (
            <label
              key={option.value}
              className={`cursor-pointer rounded-xl border p-4 transition ${selected ? 'border-teal-600 bg-teal-50 ring-2 ring-teal-100' : 'border-gray-200 bg-gray-50 hover:bg-white'
                }`}
            >
              <input
                type="radio"
...
function CommercialBasicsFields({ form, onChange, onToggle, inputClass, labelClass }) {
  return (
    <>
      <div>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          {COMMERCIAL_AMENITY_OPTIONS.map((amenity) => (
            <label key={amenity.name} className="flex items-center gap-2 rounded-lg border border-gray-200 bg-gray-50 px-4 py-3 text-sm text-gray-700">
              <input
                type="checkbox"
                name={amenity.name}
...
function CommercialMediaUploadPanel({ propertyId }) {
  return (
    <div className="rounded-xl border border-teal-100 bg-teal-50/60 p-4">
      <h3 className="text-sm font-bold text-teal-900">Commercial photos and videos</h3>
      <p className="mt-1 text-sm text-teal-800">
        Add storefront, floor-plan, utility, and access media from this Tenant & Rent tab.
      </p>
      <div className="mt-3">
        {propertyId ? (
          <ImageUploader propertyId={propertyId} initialImages={[]} onUpdate={() => { }} />
        ) : (
  ...
  return (
    <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-slate-900/45 px-4">
      <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl">
        ...
        <h2 className="mt-4 text-xl font-bold text-gray-900">Listing completed</h2>
        <p className="mt-2 text-sm leading-6 text-gray-600">
          Your {isCommercial ? 'commercial' : 'residential'} listing has been completed successfully.
        </p>
        <button
          type="button"
          onClick={onClose}
...
function getCommercialTypeLabel(value) {
  return COMMERCIAL_PROPERTY_TYPE_OPTIONS.find((option) => option.value === value)?.label || 'Commercial space';
}

function getSelectedCommercialAmenities(form) {
  return COMMERCIAL_AMENITY_OPTIONS.filter((amenity) => form[amenity.name]).map((amenity) => amenity.label);
}
...
export default function CreateListingPage() {
  ...
  const getStepRequiredFields = (stepIndex) => {
    if (stepIndex === 0 && isCommercialFlow) {
      return ['title', 'description', 'commercial_property_type', 'commercial_square_footage'];
    }
  ...
  const handleNextStep = () => {
    const validationMessage = getValidationMessage(getStepRequiredFields(currentStep));
  ...
  const handleSubmit = async (e) => {
    ...
    for (let stepIndex = 0; stepIndex < FORM_STEPS.length; stepIndex += 1) {
      const validationMessage = getValidationMessage(getStepRequiredFields(stepIndex));
      if (validationMessage) {
    ...
    try {
      ...
      const body = {
        title: form.title,
        description: isCommercialFlow ? buildCommercialDescription(form) : form.description,
        propertySizeSqft: parseFloat(isCommercialFlow ? form.commercial_square_footage : form.property_size),
        roomCount: isCommercialFlow ? 0 : parseFloat(form.room_count),
        bathroomCount: isCommercialFlow ? (form.commercial_amenity_washroom ? 1 : 0) : parseFloat(form.bathroom_count),
        ...
        {showWizard && (
          <form onSubmit={handleSubmit} onKeyDown={handleFormKeyDown} className="space-y-5" noValidate>
            <section className={sectionClass}>
              ...
              {currentStep === 0 && (
                <>
                  <RentalTypeSelector value={form.rental_type} onChange={handleChange} />
              ...
              {currentStep === 3 && (
                <>
                  <div>
                    ...
                    <select name="intended_tenant_type" value={form.intended_tenant_type} onChange={handleChange} className={inputClass} disabled={Boolean(createdPropertyId)}>
                  ...
                  <div>
                    ...
                    <input type="number" name="rent" value={form.rent} onChange={handleChange} className={inputClass} placeholder="e.g. 15000" min="0" required disabled={Boolean(createdPropertyId)} />
                  </div>

                  {isCommercialFlow && <CommercialMediaUploadPanel propertyId={createdPropertyId} />}
                </>
              )}
            </section>
            ...
            <div className="flex flex-col-reverse sm:flex-row sm:items-center sm:justify-between gap-3">
              <button
                ...
                disabled={currentStep === 0 || loading || Boolean(createdPropertyId)}
