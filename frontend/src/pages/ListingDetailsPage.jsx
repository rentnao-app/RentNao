...
function Icon({ children, className = "h-5 w-5" }) {
  return (
    <span
      className={`inline-flex shrink-0 text-emerald-700 ${className}`}
      aria-hidden
    >
      {children}
    </span>
...
export default function ListingDetailsPage() {
    ...
    void (async () => {
      try {
        ...
        const res = await apiFetch(
          `/properties/public/listings?${q.toString()}`,
        );
        ...
        setRelated(
          items
            ...
            .filter(
              (x) =>
                String(x.areaName || "").toUpperCase() ===
                String(areaKey).toUpperCase(),
            )
            .slice(0, 4),
        );
      } catch {
  ...
  useEffect(() => {
    if (!isTenant) return;
    const timer = setTimeout(() => {
      void getTenantRequestForListing(id).then((record) =>
  ...
  const listingActive =
    (listing?.listingStatus || listing?.listing_status) === "ACTIVE";
  const unlockPayment = usePaymentGuard({
    feeCode: "LISTING_UNLOCK",
    enabled: loggedIn && isTenant && Boolean(listing) && !hasAccess,
  });
  ...
  return (
    <div className="min-h-screen bg-[#f2f7f3] text-slate-800">
      ...
      <main className="mx-auto max-w-[1500px] px-3 py-4 sm:px-5 sm:py-6 lg:px-6 lg:py-8">
        ...
        <nav
          className="mb-4 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-slate-500 sm:text-sm"
        ...
        >
          ...
          <span className="text-slate-300">/</span>
          <span className="max-w-[min(100%,42rem)] truncate font-medium text-slate-700">
            {title}
          </span>
        </nav>
                  ...
                  <Icon>
                    ...
                    >
                      <path
                        ...
                        strokeLinejoin="round"
                        strokeWidth={1.8}
                        d="M5 13l4-4 4 4 6-6M5 5h14v14H5V5z"
                      />
                    </svg>
                  </Icon>
                }
                  ...
                  <Icon>
                    ...
                    >
                      <path
                        ...
                        strokeLinejoin="round"
                        strokeWidth={1.8}
                        d="M8 14v3M12 14v3M16 14v3M4 21h16M6 10h12l1 8H5l1-8zM6 10V7a2 2 0 012-2h8a2 2 0 012 2v3"
                      />
                    </svg>
                  </Icon>
                }
            ...
            {hasAccess &&
              ...
              <div className="rounded-2xl border border-slate-200/80 bg-white p-4 shadow-sm sm:p-5">
                ...
                <div className="mt-3 overflow-hidden rounded-xl border border-slate-100">
                  <MapView
                    lat={listing.exactLat}
                    lng={listing.exactLng}
                    height="220px"
                  />
                </div>
              </div>
                  ...
                  <div className="rounded-xl bg-amber-50 px-3 py-3 text-sm text-amber-950 ring-1 ring-amber-100">
                    ...
                    <button
                      ...
                      onClick={async () => {
                        ...
                        const result = await withdrawTenantRequest(
            ...
            {loggedIn && isTenant && !hasAccess ? (
              <div className="mt-4 rounded-xl border border-emerald-100 bg-emerald-50/60 p-4">
                ...
                <div className="mt-3 rounded-lg border border-emerald-100 bg-white/80 px-3 py-2 text-xs text-slate-700">
                  ...
                  {unlockPayment.error ? (
                    <p className="mt-1 text-red-600">{unlockPayment.error}</p>
                  ) : null}
                </div>
                <button
                  type="button"
                  ...
                  onClick={async () => {
                    ...
                    try {
                      const canPay =
                        await unlockPayment.ensureSufficientBalance();
                      if (!canPay) return;
                      const res = await apiFetch(
                        `/properties/listings/${id}/unlock`,
                        { method: "POST" },
                      );
                      const body = await res.json().catch(() => ({}));
                      if (
                        unlockPayment.handlePaymentRequiredResponse(res, body)
                      )
                        return;
                      if (!res.ok)
                        throw new Error(
