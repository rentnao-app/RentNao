 ...
 * Portaled to document.body so sticky/overflow ancestors cannot clip or stretch the page.
 */
export default function UserMenu({ name = '', email = '', role = '', initials = '?', avatarUrl = '' }) {
  ...
  useLayoutEffect(() => {
    ...
    const update = () => setCoords(menuPosition(wrapRef.current));
    update();
    ...
    open && coords
      ? createPortal(
          <div
            ...
            className="fixed z-[200] max-h-[min(28rem,calc(100vh-5rem))] overflow-y-auto overflow-x-hidden rounded-2xl border border-gray-100 bg-white shadow-xl ring-1 ring-black/5"
          >
...
function Icon({ path, className = 'h-4 w-4 shrink-0 text-gray-500' }) {
  return (
